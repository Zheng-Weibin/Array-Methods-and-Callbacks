...
function getFinals(data) {
  const array = data.filter((item) => item.Stage === "Final");
  return array;
}
...
function getYears(array, getFinalsCallback) {
  return getFinalsCallback(array).map((item) => item.Year);
}
...
function getWinners(array, getFinalsCallback) {
  return getFinalsCallback(array).map((item) =>
...
function getWinnersByYear(
  ...
  const winners = GetWinnersCallback(array, getFinals);
  const years = getYearsCallback(array, getFinals);
  return winners.map(
